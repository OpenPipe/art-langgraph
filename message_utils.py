"""Message conversion utilities for OpenAI and LangChain message types."""
...
def convert_langgraph_messages(messages: List[object]) -> MessagesAndChoices:
    ...
    for msg in messages:
        if hasattr(msg, 'response_metadata') and 'logprobs' in msg.response_metadata:
            converted.append(Choice(message=ChatCompletionAssistantMessageParam(role="assistant", content=msg.content), index=0, **msg.response_metadata))
